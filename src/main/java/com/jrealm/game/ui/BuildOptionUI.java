...
public class BuildOptionUI {
	...
	public BuildOptionUI() {
		this.icons = new SpriteSheet("ui/icons.png", 32, 32, 0);
		...
		this.parentSlots = this.createBuildingUI();
	}
	...
	private Slots[] createGather(int size, int x, int y, BufferedImage[] slotsHori) {
		...
		for(int i = 0; i < btnOptions.length; i++) {
			btnOptions[i] = new Button(this.icons.getSprite(i, 1).image, this.imgButton, new Vector2f(x - ((size + 10) * (i + 1)), y), size, size, 8);
			btnOptions[i].addPressedImage(this.imgPressed);
		}
	...
	private Slots[] createBuildingUI() {
		SpriteSheet slots = new SpriteSheet("ui/slots.png", 0);
		...
		Slots[] gatherSlots = this.createGather(size, buildSlotX, buildSlotY, slotsHori);
		...
		for (int i = 0; i < btnBuilding.length; i++) {
			btnBuilding[i] = new Button(this.icons.getSprite(i, 0).image, imgButton,
		...
		this.buildingButtons(buildingslots);
