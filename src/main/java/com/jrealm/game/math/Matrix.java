...
public class Matrix {
	...
	if (aMatrix[0].length != bMatrix.length) {
	    ...
	    return Matrix.identity(aMatrix);
	}
	...
	for (int i = 0; i < matrix.length; i++) {
		...
		inverse[i][j] = (float) Math.pow(-1, i + j) * Matrix.determinant(Matrix.minor(matrix, i, j));
	    }
	}

	float det = 1.0f / Matrix.determinant(matrix);
	...
	for (int i = 0; i < matrix[0].length; i++) {
	    det += Math.pow(-1, i) * matrix[0][i] * Matrix.determinant(Matrix.minor(matrix, 0, i));
	}
