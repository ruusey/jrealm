		...
		Runnable processClients = () -> {
			...
			while (!this.shutdownProcessing) {
				try {
					...
					log.error("Failed to parse client input {}", e.getMessage());
				}
			}
		};
		WorkerThread.submitAndRun(processClients);
	}
			...
			try {
				...
				if (bytesRead > 0) {
					...
					while (this.remoteBufferIndex >= 5) {
						...
						newPacket.setSrcIp(clientSocket.getInetAddress().getHostAddress());
						server.getPacketQueue().add(newPacket);
					}
				}
			} catch (Exception e) {
				log.error("Failed to parse client input {}", e.getMessage());
